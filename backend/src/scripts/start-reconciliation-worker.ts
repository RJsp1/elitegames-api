...
/**
 * Executa o ciclo de expiração no máximo uma vez por `intervalMs`.
 * Primeiro ciclo (lastExpirationRunAt === 0) pode rodar imediatamente.
 */
export async function maybeRunExpirationCycle(
  enabled: boolean,
  intervalMs: number,
): Promise<void> {
  if (!enabled || shuttingDown || expirationRunning) return;
  ...
  try {
    await paymentExpirationService.runCycle({ intervalMs });
  } catch (expireErr) {
...
export async function runOnce(intervalMs: number, batchSize?: number): Promise<void> {
  ...
  try {
    ...
    await maybeRunExpirationCycle(
      env.PAYMENT_EXPIRATION_ENABLED,
      env.PAYMENT_EXPIRATION_INTERVAL_MS,
    );
  } catch (err) {
    consecutiveFailures += 1;
    writeReconciliationHeartbeat({
